...
public class DormRepairController {
     ...
     * pageSize  显示条数
     */
    @RequestMapping(value = "/findDormRepairS")
    public String findDormRepairS(Integer d_id,String d_dormbuilding,
                                 Integer pageIndex, Integer pageSize, Model model, HttpSession session) {
        Admin ad = (Admin)session.getAttribute("ad");
        PageInfo<DormRepair> di = dormRepairService.findPageInfo(ad.getA_power(),d_dormbuilding,
                pageIndex,pageSize);
        model.addAttribute("di",di);
        return "dormrepair_listS";
    }
    ...
    @ResponseBody
    public String deleteDormRepair(Integer r_id, HttpSession session) {
        int d = dormRepairService.deleteDormRepair(r_id);
        Admin ad = (Admin)session.getAttribute("ad");
        if(ad!=null){
            if(ad.getA_power()!=1){
                return "redirect:/findDormRepairS";
            }
    ...
    public String updateDormRepair( DormRepair dormrepair, HttpSession session) {
